...
import com.android.alen.mybasicproject.Manager.EventCenterManager;
import com.android.alen.mybasicproject.Manager.GreenDaoManager;
import com.android.alen.mybasicproject.Net.AppComponent;
import com.android.alen.mybasicproject.Net.DaggerAppComponent;
import com.android.alen.mybasicproject.Net.InteractorModule;
...
public class App extends MultiDexApplication{
    public static Context mContext; // 全局Context， 方便其他代码调用
    @Inject AppComponent component; // 3.4 网络请求相关
    private static RetroInteractor retroInteractor;
    @Override
    public void onCreate() {// 这里如果处理很多逻辑， 那么应用冷启动的时间就会增加。
        super.onCreate();
        mContext = getBaseContext();

        // 3.4 获取网络请求对象
        DaggerAppComponent.builder().interactorModule(new InteractorModule()).build().inject(this);

        //6 初始化 EventCenterManager
    ...
    public static RetroInteractor getInteractor() {
        App app = (App)mContext.getApplicationContext();
        if(retroInteractor == null) retroInteractor = app.component.getRetroInteractor();
        return retroInteractor;
    }
}
